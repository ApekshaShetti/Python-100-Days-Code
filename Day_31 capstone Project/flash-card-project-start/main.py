...
def is_known():
    global score
    score+= 1;
    canvas.itemconfigure(card_score, text=f"Score= {score}")
    to_learn.remove(current_card)
    data_save = pandas.DataFrame(to_learn)
    data_save.to_csv("flash-card-project-start/data/words_to_learn.csv",index=False)
    next_card()
...
card_score = canvas.create_text(400,40,text="Score = 0",font=("Arial",15,"bold"))
...
check_img = PhotoImage(file="flash-card-project-start/images/right.png")
